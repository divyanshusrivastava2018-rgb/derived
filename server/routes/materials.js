...
router.post('/', requireAdmin, jsonParser, (req, res) => {
  ...
  const category = body.category !== undefined ? String(body.category).trim() : '';
...
router.put('/:id', requireAdmin, jsonParser, (req, res) => {
  ...
  const category =
    body.category !== undefined ? String(body.category).trim() : cur.category || '';
  items[idx] = { ...cur, title, fileUrl };
  if (category) items[idx].category = category;
  else delete items[idx].category;
  materialsStore.writeAll(items);
  res.json(items[idx]);
});
